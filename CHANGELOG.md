...
- **Cookie 真正续命**（issue #6）。以前每 10 分钟打的 `RotateCookies` 走的是浏览器
  iframe 那条 `[658, session_id]`，只刷新 `SIDCC` / `*PSIDCC`，**换不了**
  `__Secure-1PSIDTS`——那才是约 30 分钟过期的短命票，所以号看起来半小时就死、
  保活按钮也救不回来。现在先 POST 哨兵 payload `[000,"-0000000000000000000"]`
  无条件换发 `*PSIDTS`，再跑原来的 SIDCC 保活。启动 15 秒后刷第一次，不必干等
  10 分钟。同一账号 60 秒内不重复打，避免 429。

  Chrome 新版本的设备绑定会话（DBSC）这条路续不了，反复 401 时用 **Firefox**
  重新登录再导出。Firefox / 未绑定设备的会话可以一直续。
